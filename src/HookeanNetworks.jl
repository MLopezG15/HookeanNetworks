...
function Unificar(vertices1,vertices2,edges1,edges2,Kvec1,Kvec2,Kint1,Kint2;method=:x)
    ...
    if method == :y
        ...
        for i in 1:M1
            FinalArray[(N1*N1Completed):(N1+N2-1)*(N1Completed+N2Completed)]=vertices2[(N2*N2Completed+1)+1:(N2*N2Completed+1)+N2]
            N2Completed+=1
            FinalArray[(N1+N2-1)*(N1Completed+N2Completed):((N1+N2)*(N1Completed+N2Completed)+N1)]=vertices1[(N1*N1Completed+1)+1:(N1*N1Completed+1)+N1]
            N1Completed+=1
        end
        ...
        for i in eachindex(edges2) 
            ...
            FinalKvec[length(edges1)+i,:]=Kvec1[:,:]
        end
